...
void create_level_entities(Level *l, Engine *engine) {
    ...
    entityGrid = (EntityList **) calloc((size_t) l->height, sizeof(EntityList *));
    for (int j = 0; j < l->height; ++j) {
        entityGrid[j] = (EntityList *) calloc((size_t) l->width, sizeof(EntityList));
        for (int k = 0; k < l->width; ++k) {
            EntityList e_list;
            entitylist_init(5, &e_list);
            entityGrid[j][k] = e_list;
        }
    }
    ...
    for (int x = 0; x < l->height; x++) {
        for (int y = 0; y < l->width; y++) {
            ...
            if (has_door) {
                ...
                entitylist_add(&entityGrid[x][y], entityList[x][y]);
            }
            if (has_lock) {
                connectionEntityList[x][y] = create_lock_entity(engine, x, y, l->spel[x][y]);
                entitylist_add(&entityGrid[x][y], connectionEntityList[x][y]);
            }
            if (has_key) {
                entitylist_add(&entityGrid[x][y], create_key_entity(engine, x, y, l->spel[x][y]));
            }
            ...
            if (is_exit) {
                entitylist_add(&entityGrid[x][y], create_exit_entity(engine, x, y));
            }
